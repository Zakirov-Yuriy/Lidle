...
class Category {
  ...
  /// Служебная карточка «Смотреть все» в конце списка на главной.
  ///
  /// Раньше её опознавали по словам в названии (`title.contains('все')`), и на
  /// проде это сломалось: там есть каталог «Работа и вакансии ВСЕХ сфере
  /// специальностях», в котором эта подстрока встречается раньше. Поиск
  /// находил его, а настоящая кнопка исчезала. На деве названия короткие
  /// («Работа»), поэтому там всё работало. Теперь признак явный.
...
class Listing {
  ...
  /// признаку решает, что рисовать. Отдельной моделью товар в ленте не
  /// сделан намеренно: список, состояние и пагинация главной работают с
  /// одним типом, и второй тип пришлось бы протаскивать через всю цепочку
  /// ради двух отличий на карточке.
  final bool isProduct;

  /// Товар можно заказать через сайт: у раздела заведён атрибут оплаты.
  /// Без него карточка показывается, но кнопки «В корзину» нет — товар
  /// покупают на месте.
  final bool canOrder;

  /// Есть ли остаток. Нулевой остаток тоже показываем: товар вернётся.
  final bool inStock;

  /// Номер товара. У товара `id` в ленте выглядит как `product:20`, чтобы не
  /// столкнуться с объявлением с тем же номером, поэтому настоящий номер
  /// лежит отдельно.
  final int? productId;
