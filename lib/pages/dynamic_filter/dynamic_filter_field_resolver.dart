...
enum FilterFieldKind {
  ...
  /// Блок бронирования (styles L и M).
  ///
  /// Особый случай среди всех: этот атрибут НЕ хранит значения в объявлении.
  /// Он лишь метка категории «здесь можно бронировать», а настройки уезжают
  /// отдельным блоком `booking` и ложатся в свои таблицы. Поэтому его нельзя
...
FilterFieldPlan resolveFilterField(Attribute attr) {
  ...
  // Бронирование (L и M) — раньше всех остальных правил.
  //
  // У этого атрибута нет ни значений, ни типа, поэтому любое правило ниже
  // увело бы его в текстовое поле. Так и было до 03.09.2026: в форме подачи
  // рисовалась строка «Запись на просмотр» с полем «Текст».
  if (attr.style == 'L' ||
      attr.style == 'M' ||
      attr.styleSingle == 'L1' ||
      attr.styleSingle == 'M1') {
