...
class Advert {
  ...
  factory Advert.fromJson(Map<String, dynamic> json) {
    ...
    // Уменьшенные копии из СПИСКА объявлений (15.09.2026).
    //
    // В подробном ответе приходят полные снимки, в списке их нет вовсе:
    // раньше список отдавал одну обложку, и карточка в ленте листать не
    // умела. Теперь список шлёт `thumbnails`, и мы подставляем их, только
    // если полных снимков в ответе нет. Наоборот делать нельзя: на экране
    // объявления галерея стала бы размытой.
    if (imagess.isEmpty && json['thumbnails'] is List) {
      imagess.addAll(
