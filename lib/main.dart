...
void main() async {
  ...
  try {
    ...
    // Состав корзины подтягиваем В ФОНЕ, не задерживая запуск: он нужен
    // счётчикам на карточках товара, а не первому кадру. Если сеть недоступна,
    // приложение просто откроется без счётчиков, а не встанет на splash.
    unawaited(CartService.sync());
