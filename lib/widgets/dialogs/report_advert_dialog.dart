...
class ReportAdvertDialog extends StatefulWidget {
  final int advertId;
  final String advertTitle;

  const ReportAdvertDialog({
    super.key,
  ...
  @override
  State<ReportAdvertDialog> createState() => _ReportAdvertDialogState();
}

class _ReportAdvertDialogState extends State<ReportAdvertDialog> {
  ...
  /// Загружает причины жалоб на объявления с сервера
  /// (GET /content/reports?type=adverts). Сервер отдаёт {id, title}.
  Future<void> _loadReportTypes() async {
    try {
      ...
      final reasons = await ApiService.getReportReasons(type: 'adverts');
      final types = reasons
          .map((e) => {
                'id': e['id'],
