...
class _MiniPropertyDetailsScreenState extends State<MiniPropertyDetailsScreen> {
  ...
  Future<void> _loadAndShowPhoneDialog() async {
    // Вход для просмотра телефона НЕ требуется.
    //
    // Раньше здесь стояла проверка авторизации, и гостю вместо номера
    // показывалось предложение завести профиль. Это отсекало самых
    // мотивированных посетителей: человек нашёл объявление и готов
    // позвонить прямо сейчас, а мы вместо номера просим регистрацию.
    // Контакты продавца и так открыты на сайте, эндпоинт /v1/users/{id}
    // на сервере доступен без токена.
    //
    // Токен всё же читаем: он нужен ниже для отзыва о продавце. У гостя он
    // равен null, и это нормально — статистика звонков пишется и без него.
    final token = TokenService.currentToken;
    ...
    try {
      ...
      // Гостю отзыв не предлагаем: сервер примет его только от вошедшего
      // пользователя, и диалог закончился бы ошибкой.
      if (callInitiated && mounted && token != null && token.isNotEmpty) {
