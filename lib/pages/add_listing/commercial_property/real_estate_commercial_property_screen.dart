...
class RealEstateCommercialPropertyScreen extends StatelessWidget {
  final String type;

  const RealEstateCommercialPropertyScreen({super.key, required this.type});
    ...
    return Column(
      children: [
        ListTile(
          ...
          onTap: () {
            if (title == "Продажа офисов") {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => AddOfficeSellScreen(initialCategory: title)),
              );
            } else if (_getCommercialOptions("Продажа коммерческая недвижимости").contains(title)) {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => AddCommercialSellScreen(initialCategory: title)),
              );
            } else if (_getCommercialOptions("Аренда коммерческая недвижимости").contains(title)) {
              // Пока нет экрана для аренды коммерческой недвижимости, оставляем как есть или добавить позже
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => AddCommercialRentScreen(initialCategory: title)),
              );
