...
class AttributeResolver {
  ...
  /// Это критический атрибут, который должен быть в каждой категории недвижимости
  /// но с разными ID (1048, 1050, 1051, 1052, 1128, 1130, 2962 и т.д.)
  /// и возможно разными названиями в разных категориях:
  /// - "Вам предложат цену" (точное название)
  /// - "Вам предложат цену 1", "Вам предложат цену 2" и т.д. (с суффиксом)
  ///
  /// ВАЖНО: Возвращает null если атрибута нет, не ищет fallback!
  /// Для Jobs и других категорий без "Вам предложат цену" возвращаем null
  int? getOfferPriceAttributeId() {
    // Сначала ищем по ТОЧНОМУ названию (базовый случай)
    var id = findAttributeIdByName('Вам предложат цену');
    if (id != null) return id;

    // Если не нашли точно, ищем по частичному совпадению (с суффиксами)
    // Это позволит найти "Вам предложат цену 1", "Вам предложат цену 2" и т.д.
    // без необходимости добавлять каждый ID в список известных
    id = findAttributeIdByPartialName('предложат цену');
    if (id != null) return id;

    // Если не нашли, ищем по более общему ключевому слову
    id = findAttributeIdByPartialName('предложат');
    if (id != null) return id;

    // Для Job категорий и других без "Вам предложат цену" возвращаем null
  ...
  int? getAreaAttributeId() {
    // Только по названию (22.09.2026).
    //
    // Раньше, если «площади» не находилось, бралось первое обязательное
    // целое «от/до». В «Бронировании / Рестораны» это «Средняя сумма чека»:
    // форма считала её площадью и перед отправкой подменяла введённое
    // «1500–3000» на {value: 50} без «до», и сервер отвечал «требуется
    // указать максимальное значение». Площадь у недвижимости всегда
    // называется «…площадь», угадывать по типу не нужно.
    return findAttributeIdByPartialName('площадь');
  }
