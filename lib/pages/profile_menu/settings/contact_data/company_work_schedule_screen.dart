...
//
// РАБОЧИХ ДНЕЙ ЗДЕСЬ ПОКА НЕТ. На макете под подписью пустое место, и как
// выбираются дни, ещё не нарисовано. Поле в хранении под них уже оставлено,
// так что когда макет появится, сервер трогать не придётся.

import 'package:flutter/material.dart';
import 'package:lidle/widgets/components/header.dart';

/// Что экран возвращает: начало и конец рабочего дня в виде «ЧЧ:ММ».
/// Пусто — время не задавали.
class WorkScheduleChoice {
  const WorkScheduleChoice({this.start, this.end});
...
class CompanyWorkScheduleScreen extends StatefulWidget {
  const CompanyWorkScheduleScreen({super.key, this.start, this.end});
...
class _CompanyWorkScheduleScreenState extends State<CompanyWorkScheduleScreen> {
  ...
  void _done() {
    Navigator.pop(context, WorkScheduleChoice(start: _start, end: _end));
  }
        ...
        body: SafeArea(
          child: Column(
            ...
            children: [
              ...
              // Место под выбор рабочих дней. На макете оно пустое, и как
              // именно выбираются дни, ещё не нарисовано: рисовать за
              // заказчика значит показать продавцу список, который никто не
              // согласовывал.
              const Spacer(),
