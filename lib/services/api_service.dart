...
class ApiService {
  ...
  static Future<Map<String, dynamic>> _getRequest(
    ...
    try {
      ...
      // Текст исключения обязателен. Раньше здесь было просто «Неизвестная
      // ошибка», и настоящая причина пропадала совсем: в логах оставалась
      // строка, по которой невозможно понять, разбор ответа это, права,
      // сертификат или что-то ещё. За один день это дважды стоило нам
      // получаса гаданий.
  ...
  static Map<String, dynamic> _handleResponse(http.Response response) {
    ...
    if (response.statusCode >= 200 && response.statusCode < 300) {
      ...
      // 409 = конфликт. В бронировании это «время заняли, пока вы заполняли
      // форму»: данные верные, ругаться не на что, надо перечитать календарь
      // и дать выбрать заново. Отличить его от 422 по одному лишь тексту
      // нельзя, поэтому кладём код в тело ответа.
      return {...data, 'status_code': 409};
