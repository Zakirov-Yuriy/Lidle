...
class _RealEstateListingsScreenState extends State<RealEstateListingsScreen> {
  ...
  Future<void> _loadAdverts({
    ...
    try {
      ...
      //
      // Этот экран ходит в ручку объявлений, а товары лежат в своей таблице и
      // в её выдачу не попадают. Раздел с товарами показывал «Пока объявлений
      // нет», хотя товары в нём есть.
      //
      // Подмешиваем их ПОСЛЕ клиентских фильтров и ДО сортировки: фильтры
      // написаны по полям объявления (город берётся из адреса, характеристики
      // из ответа объявлений), и товар отсеялся бы как «без адреса». А вот
      // сортировка по цене и дате должна работать на общий список, иначе
      // товары всегда стояли бы отдельной кучей.
      //
      // Только первая страница: подгрузка следующей листает объявления, и
      // повторять товары на каждой странице значит показать их по разу на
      // экран.
      if (!isNextPage) {
        ...
        log.d('🛒 Товаров в разделе: ${products.length}');

        if (products.isNotEmpty) {
          sortedNewListings = <Listing>[...products, ...sortedNewListings];
        }
      }
