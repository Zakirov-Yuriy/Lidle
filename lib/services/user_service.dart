...
class UserService {
  ...
  /// Сбросить город в профиле.
  ///
  /// Нужно потому, что город из профиля задаёт приоритет ленты на главной
  /// (задача 70). Человек мог указать Мариуполь, а работать по Москве, и
  /// тогда лента подсовывает ему не то. На сайте такая кнопка уже есть, в
  /// приложении её не было, и логика расходилась.
  ///
  /// Присылаем именно `null`, а не пустую строку: бэк на `city_id: null`
  /// очищает город вместе с улицей и домом (без города «улица Ленина»
  /// осталась бы неизвестно где), а отсутствие ключа ничего не меняет.
  static Future<Map<String, dynamic>> clearAddress({
    required String token,
  }) async {
    try {
      ...
      throw Exception('Ошибка при сбросе города: $e');
    }
  }
