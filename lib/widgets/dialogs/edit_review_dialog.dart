...
class _EditReviewDialogState extends State<EditReviewDialog> {
  ...
  Future<void> _submit() async {
    ...
    // Раньше здесь считалось, что null — это ошибка. На деле было наоборот:
    // при 422 метод возвращал тело ответа (не null), и правка «сохранялась»,
    // хотя бэк её отклонил. Теперь null означает успех, а строка — причину.
    if (result != null) {
      setState(() {
        _submitting = false;
        _error = result;
