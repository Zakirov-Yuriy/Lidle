...
class _SellerProfileScreenState extends State<SellerProfileScreen> {
  ...
  /// продавца вместо вывески. Теперь аватар магазина берётся ТОЛЬКО отсюда,
  /// а нет логотипа — рисуется общая заглушка.
  ///
  /// `widget.sellerAvatarUrl` для самой вывески больше не используется: он
  /// остаётся у дочерних экранов, которым передаётся дальше.
  ...
  Widget _buildSellerInfo() {
    return Column(
      children: [
        Row(
          children: [
            // ТОЛЬКО логотип компании (правка заказчика 18.09.2026).
            //
            // Раньше экран рисовал картинку, переданную снаружи, а там в
            // зависимости от места оказывался то логотип точки, то аватар
            // самого человека, и в магазине висело лицо владельца. Запасным
            // вариантом аватар тоже не годится: нет логотипа — пусть стоит
            // заглушка, это честнее, чем подменять вывеску человеком.
            _buildSellerAvatar(_companyImage),
  ...
  /// Ряд иконок «Поделиться». Вид и поведение вынесены в общий виджет
  /// ShareIconsRow, чтобы на карточке объявления был ровно такой же блок.
  Widget _buildShareIconsRow() {
    return ShareIconsRow(
      url: _generateSellerProfileUrl(),
      text: 'Профиль продавца ${widget.sellerName} на LIDLE',
    );
  }
