...
class _ProductItemsScreenState extends State<ProductItemsScreen> {
  ...
  Future<void> _addPosition() async {
    ...
    // Заведённая позиция НЕ опубликована, и человек должен увидеть это сразу,
    // а не обнаружить через неделю, что товара нет в продаже. Поэтому сразу
    // после «Сохранить» показываем сводку с кнопкой «Опубликовать» —
    // требование заказчика от 14.09.2026.
    //
    // Правка позиции сюда не ведёт: там человек занят одной карточкой и
    // возвращается к списку, а не заканчивает работу.
    await _openReview();
  }
        ...
        Expanded(
          child: ListView(
            ...
            children: [
              ...
              // Пока есть что публиковать, говорим об этом прямо здесь.
              //
              ...
              //
              // Условие не `isPublished`, а `hasUnpublished`: витрина может
              // быть опубликована на прошлой неделе, а добавленная сегодня
              // позиция всё ещё ждать кнопки. Поймали 14.09.2026.
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(
            child: Text(
              // Говорим по-разному: «ничего не опубликовано» и «новое ждёт
              // публикации» это разные новости, и вторую человек иначе
              // пропустит — витрина-то у него уже работает.
              _publication.isPublished
                  ? 'Новые позиции ещё не опубликованы: '
                        '${_publication.unpublishedCount} шт. Покупатели их '
                        'не видят, пока вы не нажмёте «Опубликовать».'
